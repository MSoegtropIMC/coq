...
IF "%~0" == "-develop" (
  SET DEVELOP=%~1
  CALL :CheckYN -develop %~1 || GOTO ErrorExit
  SHIFT
  SHIFT
  GOTO Parse
)
...
IF "%DEVELOP%"=="Y" (
  SET CYGWIN_OPT= %CYGWIN_OPT% -P emacs-w32,emacs-ocaml
)
...
:PrintPars
  ...
  ECHO -develop  ^<Y or N^> optimize setup for coq development
  ECHO(
